...
export class CommentParser extends EventEmitter {
    ...
    constructor(
        public inputStream: fs.ReadStream,
        private parseOptions = defaultParserOptions
    ) {
        super();
        this.matchers = {
            blockStartRegex: new RegExp(
                `${parseOptions.commentStartRegex}\\s+InGenR:expand\\s+(\\S*)\\s*($|${parseOptions.commentEndRegex})`
            ),
